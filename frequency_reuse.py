...
class FrequencyReuse(Tk):
    ...
    def __init__(self, cluster_size, i, j, columns=16, rows=10, edge_len=30):
        Tk.__init__(self)
        self.i = i
        self.j = j
        self.textbox = None
    ...
    def show_lines(self):
        ...
        for k in range(1, len(self.co_cell_endp)):
            ...
            l_id = self.canvas.create_line(
                approx_center[0], approx_center[1],
                end_xx, end_yy
            )
            if self.j == 0:
    ...
    def is_within_bound(self, coords):
        if self.TOP_LEFT[0] < coords[0] < self.BOTTOM_RIGHT[0] and \
           self.TOP_RIGHT[1] < coords[1] < self.BOTTOM_RIGHT[1]:
            return True
        return False
    ...
    def call_back(self, evt):
        ...
        if self.first_click:
            ...
            for _ in range(6):
                end_xx = approx_center[0] + self.center_dist * self.i * cos(radians(self.curr_angle))
                end_yy = approx_center[1] + self.center_dist * self.i * sin(radians(self.curr_angle))

                reuse_x = end_xx + (self.center_dist * self.j) * cos(radians(self.curr_angle - 60))
                reuse_y = end_yy + (self.center_dist * self.j) * sin(radians(self.curr_angle - 60))
            ...
            if curr in self.reuse_list:
                self.canvas.itemconfigure(hexagon.tags, fill="green")
                self.write_text(f"Correct! Cell {hexagon.tags} is a co-cell.")
                if self.curr_count == len(self.reuse_list) - 1:
                    self.write_text("Great! Press Shift-R to restart")
                    self.show_lines()
                self.curr_count += 1
            else:
                self.write_text(f"Incorrect! Cell {hexagon.tags} is not a co-cell.")
                self.canvas.itemconfigure(hexagon.tags, fill="red")


if __name__ == '__main__':
    print("Enter i & j values. Common (i,j) values are: (1,0), (1,1), (2,0), (2,1), (3,0), (2,2)")
    i = int(input("Enter i: "))
    j = int(input("Enter j: "))
    if i == 0 and j == 0:
